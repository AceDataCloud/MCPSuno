...
Environment Variables:
  ...
  LOG_LEVEL                  Logging level (default: INFO)
    ...
    safe_print("  Loading tools...")
    import tools  # noqa: F401

    safe_print("  [OK] Tools loaded")
    safe_print("")
    safe_print("  Available tools:")
    safe_print("    - generate_music")
    safe_print("    - generate_custom_music")
    safe_print("    - extend_music")
    safe_print("    - cover_music")
    safe_print("    - concat_music")
    safe_print("    - generate_with_persona")
    safe_print("    - generate_lyrics")
    safe_print("    - create_persona")
    safe_print("    - get_task")
    safe_print("    - get_tasks_batch")
    safe_print("    - list_models")
    safe_print("    - list_actions")
    safe_print("    - get_lyric_format_guide")
    safe_print("")
    safe_print("=" * 50)
